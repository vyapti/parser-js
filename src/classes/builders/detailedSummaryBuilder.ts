...
class DetailedSummaryBuilder extends SummaryBuilder {
  ...
  public build(): DetailedSummary {
    const branchRecord = new DetailedRecord(this._branchTotal, this._branchHits, undefined, this._branchDetails);
    const functionRecord = new DetailedRecord(this._functionTotal, this._functionHits, undefined, this._functionDetails);
    const lineRecord = new DetailedRecord(this._lineTotal, this._lineHits, undefined, this._lineDetails);
    return new DetailedSummary(this._filePath, this._name, branchRecord, functionRecord, lineRecord);
  }
  ...
  public parse(line: string): void {
    ...
    switch (type) {
      case InfoTypes.FunctionName: {
        ...
        const existingIdx = this._functionDetails.findIndex(fd => fd.name === name);
        if (existingIdx >= 0) {
          const { executionCount } = this._functionDetails[existingIdx];
          this._functionDetails[existingIdx] = new FunctionDetail(lineNo, executionCount, name);
        } else {
      ...
      case InfoTypes.FunctionNameCovered: {
        ...
        if (existingIdx >= 0) {
          ...
          this._functionDetails[existingIdx] = new FunctionDetail(lineNumber, executionCount, name);
        } else {
          this._functionDetails.push(new FunctionDetail(-1, executionCount, name));
        }
        break;
      }
      case InfoTypes.CoveredLine: {
        ...
        const existingIdx = this._lineDetails.findIndex(d => d.lineNumber === lineNo);
        if (existingIdx >= 0) {
      ...
      case InfoTypes.BranchCovered: {
        const [lineNo, blockNo, branchNo, execCount] = content.split(',').map(this._convertToInt);
        const existingIdx = this._branchDetails.findIndex(bd => bd.lineNumber === lineNo && bd.blockNumber === blockNo);
        if (existingIdx >= 0) {
          this._branchDetails[existingIdx].addBranchExecutionCount(branchNo, execCount);
        } else {
